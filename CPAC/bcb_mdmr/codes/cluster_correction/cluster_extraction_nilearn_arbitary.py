...
def process_nii_file(temp_dir, permut_index, mask_img):
    ...
    try:
        ...
        table = get_clusters_table(masked_one_minus_p_img, stat_threshold=0.999, cluster_threshold=0)
...
def analyze_clusters(group, variable, smoothness, mask_img, vox_threshold, cluster_threshold_alpha):
    ...
    naive_table, naive_label_maps = get_clusters_table(
        masked_one_minus_p_img,
        stat_threshold=one_minus_p_threshold,  # 이 값은 그대로 유지
        cluster_threshold=0,  # cluster_threshold 없이 진행
        return_label_maps=True
    )

    # Cluster 테이블이 비어있지 않다면, max cluster size를 계산하고 출력
    if not naive_table.empty:
        naive_table['Cluster Size (mm3)'] = pd.to_numeric(naive_table['Cluster Size (mm3)'], errors='coerce')
        max_cluster_size = naive_table["Cluster Size (mm3)"].max()
        
        # max_cluster_size가 NaN일 경우 0으로 설정
        if pd.isna(max_cluster_size):
            max_cluster_size = 0
            
        print(f"Max cluster size: {max_cluster_size/64}")
    else:
...
def main():
    ...
    parser.add_argument('--smoothness', type=int, default=8, help="Smoothness value (default: 6).")
